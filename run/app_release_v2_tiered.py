# =====================================================================
# FAIRITHM release app v2 — 3-TIER FLAG (RED / YELLOW / GREEN)
# ---------------------------------------------------------------------
# POST-STUDY UI IMPROVEMENT. This version differs from the version used
# in the user study (app_release_v1.py) in ONE way only:
#
#   To surface WEAK bias signals that the binary flag misses, the flag
#   display uses a 3-tier (red / yellow / green) scheme:
#     - RED    : aggregated signal >= 0.2  (identical to the study rule;
#                unchanged, so RED here == RED in the paper/study)
#     - YELLOW : 0.1 <= aggregated signal < 0.2  ("caution" band — a weak
#                unlearning signal was detected; review recommended)
#     - GREEN  : aggregated signal < 0.1  (no strong signal; NOT a
#                guarantee that the sentence is bias-free)
#
# Rationale: some bias categories (e.g., gender) carry weak signals under
# the current unlearning checkpoint (e.g., "여성은 운전을 못합니다." scores
# ~0.12 → green under the binary rule). Lowering the binary threshold to
# 0.1 would catch these but raises the false-positive rate to ~.50
# (alarm fatigue). The yellow caution band communicates weak signals
# honestly without inflating red false alarms.
#
# The underlying detection rule (signed positive PLL drops, top-k mean
# with k = max(3, ceil(0.2 * n_pos))) is IDENTICAL to the paper. Only the
# UI mapping of the signal to display tiers is extended.
# History is written to history_v2.jsonl to keep study records separate.
# =====================================================================
import os
import json
import re
...
# v2 uses its own history file so study records in history.jsonl stay untouched
HISTORY_FILE = str((Path(__file__).resolve().parent / "history_v2.jsonl"))
...
st.set_page_config(page_title="FAIRITHM v2 (3-Tier)", layout="wide")
...
with st.sidebar:
    ...
    st.markdown('<div class="sidebar-title">FAIRITHM <span style="font-size:14px;color:#d97706;">v2 · 3-Tier</span></div>', unsafe_allow_html=True)
...
if page in ("FAIRITHM", "BIAS CHECKER"):
    ...
    # Title (v2: distinguishable from the study version)
    st.title("FAIRITHM v2 : Bias Checker & Corrector")
    st.markdown(
        """
        <div style='display:inline-flex;align-items:center;gap:8px;
                    background:#fffbeb;border:1px solid #fcd34d;border-radius:9999px;
                    padding:4px 14px;margin-bottom:12px;font-size:13px;font-weight:600;color:#92400e;'>
            🚦 3-Tier flags (Red / Yellow / Green) — post-study improved UI
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    A **yellow** flag marks weak bias signals that the binary (study) version would not flag.
    """)
...
"""
                ...
                # v2: Non-word tokens (e.g., punctuation) and tokens with zero drop are excluded from the display
                bias_words_list = []
                if bias_tokens_list:
                    for bt in bias_tokens_list:
                        if len(bias_words_list) >= 3:
                            break
                        ...
                        if token_text:
                            ...
                            if not re.search(r"[0-9A-Za-z가-힣]", cleaned_text):
                                continue
                ...
                # Sentence signal = average of top-k positive drops, k = max(3, ceil(0.2 * 양수 토큰 수)))
                ...
                # v2: 3-tier mapping — The RED threshold remains at 0.2 according to the study rule, 
                # while the 0.1–0.2 interval is additionally marked as YELLOW (caution) to indicate weaker signals
                BIAS_THRESHOLD = 0.2      # study rule, unchanged
                CAUTION_THRESHOLD = 0.1   # v2 caution band lower bound
                if aggregated_signal >= BIAS_THRESHOLD:
                    tier = "red"
                elif aggregated_signal >= CAUTION_THRESHOLD:
                    tier = "yellow"
                else:
                    tier = "green"
                has_bias = tier == "red"
                ...
                if tier in ("red", "yellow") and bias_words_list:
                    flag_words = ", ".join(bias_words_list[:2])
                ...
                # v2 Traffic light style: Red / Yellow / Green 
                def _flag_colors(active, palette):
                    if active:
                        return palette
                    return {"bg": "#f3f4f6", "text": "#6b7280", "border": "#d1d5db", "dot": "#9CA3AF"}

                red_c = _flag_colors(tier == "red", {"bg": "#fee2e2", "text": "#dc2626", "border": "#b91c1c", "dot": "#dc2626"})
                yellow_c = _flag_colors(tier == "yellow", {"bg": "#fef9c3", "text": "#a16207", "border": "#ca8a04", "dot": "#eab308"})
                green_c = _flag_colors(tier == "green", {"bg": "#d1fae5", "text": "#059669", "border": "#047857", "dot": "#059669"})

                red_sub = bias_message if tier == "red" else "No strong bias detected"
                yellow_sub = (
                    f"Weak signal ({aggregated_signal:.2f}) — review recommended. {bias_message}"
                    if tier == "yellow" else "No weak-signal caution"
                )
                green_sub = bias_message if tier == "green" else "Signal detected — see flag on the left"

                def _flag_box(colors, title, sub):
                    return f"""
                        <div style='
                            ...
                            min-height: 96px;
                        '>
                            <span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:{colors["dot"]};box-shadow:0 0 0 2px rgba(0,0,0,0.05)'></span>
                            <div style='flex: 1;'>
                                <div style='font-size: 15px; font-weight: 700; color: {colors["text"]}; margin-bottom: 4px;'>
                                    {title}
                                </div>
                                <div style='font-size: 12px; color: {colors["text"]}; opacity: 0.9;'>
                                    {sub}
                                </div>
                            </div>
                        </div>
                ...
                with col_red:
                    st.markdown(_flag_box(red_c, "Bias detected", red_sub), unsafe_allow_html=True)
                with col_yellow:
                    st.markdown(_flag_box(yellow_c, "Caution: weak signal", yellow_sub), unsafe_allow_html=True)
                with col_green:
                    st.markdown(_flag_box(green_c, "No strong signal", green_sub), unsafe_allow_html=True)

                st.caption(
                    f"Aggregated signal: **{aggregated_signal:.3f}**  ·  tiers — red ≥ {BIAS_THRESHOLD}, "
                    f"yellow {CAUTION_THRESHOLD}–{BIAS_THRESHOLD}, green < {CAUTION_THRESHOLD}"
                )
